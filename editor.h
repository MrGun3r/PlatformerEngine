   ...
   for(int i = 0;i<sizeof(buttons)/sizeof(buttons[0]);i++){
    if(buttons[i].reserved){
      ...
      if(mouse.x >= xMin && mouse.x <= xMax && mouse.y >= yMin && mouse.y <= yMax && mouse.left == -1){
        mouse.left = 0;
       if(i == 0){
         addPlatform(windowWidth/2-camera.x,windowHeight/2-camera.y,25,25,0,false,1,0,50,0,0);
       } 
       if(i == 1 && editor.selected){
         ...
         addPlatform(windowWidth/2-camera.x,windowHeight/2-camera.y,25,25,0,false,-1,1,50,0,0);
       }
       else if (i == 14){
         addPlatform(windowWidth/2-camera.x,windowHeight/2-camera.y,25,25,0,false,-1,2,50,0,0);
       }
       else if (i == 15){
         camera.scaleReal *= 1.2;
         camera.scaleReal = min(4,camera.scaleReal);
...
void FDraw_Editor(){  
   ...
   for(int i = (int)(camera.x-windowWidth/2)%((int)(20*camera.scale));i<windowWidth;i+=20*camera.scale){
      SDL_RenderDrawLine(renderer,i,0,i,windowHeight);
   }
   for(int j =  (int)(camera.y-windowHeight/2)%((int)(20*camera.scale));j<windowHeight;j+=20*camera.scale){
...
void FDraw_SideBar_Editor(){
  ...
  if(editor.selected){
   ...
   char textureScale[10];
   if(editor.typeSelected == 0){
      renderText(sizeof("Player"),"Player",windowWidth-125,10,sizeof("Player")*12,15,255,200,(int[3]){255,255,255});
   ...
   if(editor.typeSelected == 1){
      ...
      sprintf(textureScale,"%d\0",(int)platforms[editor.indexSelected].textureScale);
      renderText(len(textureOffsetx),textureOffsetx,windowWidth-40,312,len(textureOffsetx)*10,10,255,200,(int[3]){255,255,255});
      renderText(len(textureOffsety),textureOffsety,windowWidth-40,332,len(textureOffsety)*10,10,255,200,(int[3]){255,255,255});
      renderText(len(textureScale),textureScale,windowWidth-35,352,len(textureScale)*10,10,255,200,(int[3]){255,255,255});
      if(platforms[editor.indexSelected].type == 1){
       renderText(sizeof("Finish"),"Finish",windowWidth-150,10,sizeof("Finish")*12,15,255,200,(int[3]){255,255,255});
      }
      else if (platforms[editor.indexSelected].type == 2){
       renderText(sizeof("Checkpoint"),"Checkpoint",windowWidth-150,10,sizeof("Checkpoint")*12,15,255,200,(int[3]){255,255,255});
      }
      else {
      renderText(sizeof("Platform"),"Platform",windowWidth-150,10,sizeof("Platform")*12,15,255,200,(int[3]){255,255,255});
      ...
      renderText(len(slope),slope,windowWidth-40,252,len(slope)*10,10,255,200,(int[3]){255,255,255});
      renderText(len(texture),texture,windowWidth-40,272,len(texture)*10,10,255,200,(int[3]){255,255,255});
      renderText(len(SlopeInv),SlopeInv,windowWidth-40,292,len(SlopeInv)*10,10,255,200,(int[3]){255,255,255});
      }
   ...
   if (editor.typeSelected == 2){
     ...
     renderText(sizeof("Trigger"),"Trigger",windowWidth-150,10,sizeof("Trigger")*12,15,255,200,(int[3]){255,255,255});
...
void FTransformState(){
  if(mouse.left && editor.selected && !((mouse.y <= 100 && mouse.x > 0 && mouse.x < windowWidth-200) || (mouse.x > windowWidth-200))){
   ...
   if(editor.typeSelected == 0){
      ...
      player[0].width += (float)mouse.dX/camera.scale;
      player[0].height += (float)mouse.dY/camera.scale;
