...
void FtexturePlatform(int platformID){
  ...
  // Texturing currently uses CPU , Which is stupidily inefficient , Waiting for SDL to add Texture UV Wrapping...
     ...
     if(!platforms[platformID].textureStretch){
       ...
       double i_start = platforms[platformID].xDraw-platforms[platformID].textureOffsetX*textureScale/100;
       double j_start = platforms[platformID].yDraw-(platforms[platformID].heightDraw*sin(platforms[platformID].slope*(platforms[platformID].slope>0))) - platforms[platformID].textureOffsetY*textureScale/100;
       double j_max = platforms[platformID].yDraw + platforms[platformID].heightDraw;
       double i_max = platforms[platformID].xDraw + platforms[platformID].widthDraw;

       for(double i = i_start; i< i_max ;i += textureScale){
        for(double j = j_start;  j < j_max ;j += textureScale){
          ...
          SDL_Point points[5]; 
           ...
           //printf("x :%f %f y :%f %f %f\n",xTexture0,xTexture1,yTexture0,yTexture1,yTexture2);
           
           double UV_Sources[4][2] = {{xTexture0, yTexture0},{xTexture1, yTexture1},{xTexture1, yTexture2},{xTexture0, yTexture2}};

           int textureRotate = platforms[platformID].textureRotation;

           SDL_Vertex vertex_1 = {{(int)points[0].x, (int)points[0].y}, {(int)platforms[platformID].red, (int)platforms[platformID].green, (int)platforms[platformID].blue, (int)platforms[platformID].opacity}, {UV_Sources[textureRotate %4][0], UV_Sources[textureRotate %4][1]}};
           SDL_Vertex vertex_2 = {{(int)points[1].x, (int)points[1].y}, {(int)platforms[platformID].red, (int)platforms[platformID].green, (int)platforms[platformID].blue, (int)platforms[platformID].opacity}, {UV_Sources[(textureRotate+1)%4][0], UV_Sources[(textureRotate+1)%4][1]}};
           SDL_Vertex vertex_3 = {{(int)points[2].x, (int)points[2].y}, {(int)platforms[platformID].red, (int)platforms[platformID].green, (int)platforms[platformID].blue, (int)platforms[platformID].opacity}, {UV_Sources[(textureRotate+2)%4][0], UV_Sources[(textureRotate+2)%4][1]}};
           SDL_Vertex vertex_4 = {{(int)points[3].x, (int)points[3].y}, {(int)platforms[platformID].red, (int)platforms[platformID].green, (int)platforms[platformID].blue, (int)platforms[platformID].opacity}, {UV_Sources[(textureRotate+3)%4][0], UV_Sources[(textureRotate+3)%4][1]}}; 
