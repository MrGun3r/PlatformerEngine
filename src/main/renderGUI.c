...
void renderSliders(){
  for(int i = 0;i<sizeof(sliders)/sizeof(sliders[0]);i++){
    if(sliders[i].reserved){
      ...
      // Icon for the slider (Never used but maybe will :D )
      int offset = 0;
      ...
      // Add Text Blinker if typing in the slider
      if(app.sliderTextBoxSelected == i){
        renderText(1,"-",sliders[i].x+offset+10+sliders[i].textSize*sliders[i].ButtonFontWidth + sliders[i].sliderLength+len(sliders[i].value)*sliders[i].ButtonFontWidth,sliders[i].y,sliders[i].ButtonFontWidth,sliders[i].ButtonFontHeight,255*(1+sin(app.TextBlink))/2,255*(1+sin(app.TextBlink))/2,(int[3]){255,255,255});
      }
...
void renderKnobs(){
  for(int i = 0;i<sizeof(knobs)/sizeof(knobs[0]);i++){
    if(knobs[i].reserved){
      ...
      // Add Text Blinker if typing in the knob
      if(app.knobTextBoxSelected == i){
        renderText(1,"-",knobs[i].x+offset+knobs[i].textSize*knobs[i].ButtonFontWidth+25+len(knobs[i].value)*knobs[i].ButtonFontWidth,knobs[i].y,knobs[i].ButtonFontWidth,knobs[i].ButtonFontHeight,255*(1+sin(app.TextBlink))/2,255*(1+sin(app.TextBlink))/2,(int[3]){255,255,255});
      }
    }
  }
}
