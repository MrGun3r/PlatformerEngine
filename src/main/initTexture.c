  ...
  if (p != NULL)
  {
    while ((pp = readdir (p))!=NULL) {
      ...
      if (FCompareStrings(FlowerCase(&(pp->d_name[length-4]),4),".png\0")) {
         ...
         tempName[tempNameSize] = '\0';
         char path[256];
         SDL_memcpy(lightTextures[index].textureName,tempName,tempNameSize+1);
         sprintf(path,"assets/light/%s\0",pp->d_name);
         printf("Loaded %s\n",path);
         SDL_Surface *texture_Surface = IMG_Load(path);
         SDL_Texture *texture = SDL_CreateTextureFromSurface(renderer,texture_Surface); 
         lightTextures[index].texture = texture;
         lightTextures[index].reserved = true;
         index++;
         SDL_FreeSurface(texture_Surface);
      }
    }
...
void FLoadBackgrounds(){
  ...
  for(int i = 0;i<sizeof(backgrounds)/sizeof(backgrounds[0]);i++){
   ...
   surface_font = IMG_Load("assets/fonts/font.png");
   ...
   surface_script = IMG_Load("assets/editor/script.png");
   surface_enemy = IMG_Load("assets/enemy/enemy.png");
   surface_explosion = IMG_Load("assets/editor/explosion.png");
   surface_specials = IMG_Load("assets/player/specials.png");
   surface_textIcon = IMG_Load("assets/editor/textIcon.png");
   surface_grappleFail = IMG_Load("assets/player/grappleFailed.png");
   ...
   tex_script = SDL_CreateTextureFromSurface(renderer,surface_script);
   tex_enemy = SDL_CreateTextureFromSurface(renderer,surface_enemy);
   tex_explosion = SDL_CreateTextureFromSurface(renderer,surface_explosion);
   tex_specials = SDL_CreateTextureFromSurface(renderer,surface_specials);
   tex_textIcon = SDL_CreateTextureFromSurface(renderer,surface_textIcon);
   tex_grappleFail = SDL_CreateTextureFromSurface(renderer,surface_grappleFail);
   ...
   SDL_FreeSurface(surface_movenode);
   SDL_FreeSurface(surface_script);
   SDL_FreeSurface(surface_explosion);
   SDL_FreeSurface(surface_enemy);
   SDL_FreeSurface(surface_specials);
   SDL_FreeSurface(surface_textIcon);
   SDL_FreeSurface(surface_grappleFail);
